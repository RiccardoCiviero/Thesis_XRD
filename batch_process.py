...
if not os.path.isdir(str(args.path) + "/Projected"):
    os.makedirs(str(args.path) + "/Projected")
...
for root, dirs, files in os.walk(args.path + "/Omega"):
    for name in files:
        if "w-scans" in name:
            f = root + "/" + name
            subprocess.run(['python', 'xrd-rsm-project.py', f, '-s', str(args.startnum), '-e', str(args.endnum), '-d', str(args.path) +"/Projected"], stdout=open(os.devnull, 'wb'))
